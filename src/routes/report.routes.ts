...
router.post('/generate', authenticate, requirePermission('REPORT_GENERATE'), asyncHandler(async (req, res) => {
...
// Save BFMP draft
router.post('/bfmp/draft', authenticate, requirePermission('REPORT_GENERATE'), asyncHandler(async (req, res) => {
...
router.get('/preview/:workOrderId', authenticate, asyncHandler(async (req, res) => {
...
// Supports ?type=inspection (default) or ?type=work-order
router.get('/view/:workOrderId', authenticate, asyncHandler(async (req, res) => {
  if (!(await assertReportAccess(req, res))) return;
  const reportType = (req.query.type as string) || 'inspection';
...
router.get('/config/:workOrderId', authenticate, asyncHandler(async (req, res) => {
  if (!(await assertReportAccess(req, res))) return;
  const data = await reportService.getInspectionReportConfig(req.params.workOrderId as string);
  res.json({ success: true, data });
}));

router.put('/config/:workOrderId', authenticate, asyncHandler(async (req, res) => {
...
router.get('/context/:workOrderId', authenticate, asyncHandler(async (req, res) => {
  if (!(await assertReportAccess(req, res))) return;
